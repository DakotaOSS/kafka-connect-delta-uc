...
The token principal needs `EXTERNAL USE SCHEMA` on the target schema. Route one task per table (topic→table) so writers don't collide; UC conflict arbitration is the safety net, not the primary concurrency strategy.
...
| `table.name.format` | `main.ingestion.${topic}` | Three-part UC name; `${topic}` is substituted with the sanitised topic. |
